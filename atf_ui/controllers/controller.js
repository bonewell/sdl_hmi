        ...
        function(err, doc) {
          if (!err && doc) {
            ...
            for (var i in luaFiles) {
              ...
              err = fs.unlinkSync(luaFiles[i].path);
            ...
            for (var i in otherFiles) {
              ...
              fs.unlink(otherFiles[i].path, function(err) {
                ...
                if (err) {
                  ...
                  console.log('----------saved new file', otherFiles[i].originalname);
                }
              });
            }
            ...
            for (var i in modules_upload) {
              ...
              fs.unlink(modules_upload[i].path, function(err) {
                ...
                if (err) {
                  ...
                  console.log('----------saved new file', modules_upload[i].originalname);
                }
              });
            }
...
controller.ajax = function(req, res) {
  ...
  switch (req.body.objectData) {
    case 'deleteFile': {
      ...
      req.db.collection('tests').findOne({userID: req.session.user._id, testSuite: req.body.data.suit},
                function(err, doc) {
                  if (err) {
                    res.status(500).send('Data base internal error;', err);
                  } else {
                    var array = doc.fileNames;
                    var itemIndex = array.indexOf(req.body.data.fileName);
                    var path = testSuitePath.concat(req.session.user._id.concat('/' +
                        doc._id.concat('/' + req.body.data.fileName)));
